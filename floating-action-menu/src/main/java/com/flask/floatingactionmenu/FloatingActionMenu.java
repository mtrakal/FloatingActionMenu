...
public class FloatingActionMenu extends ViewGroup implements OnToggleListener {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        for (int i = fabList.size() - 1; i >= 0; i--) {
            ...
            if (rtl) {
                fab.layout(0, y, fab.getMeasuredWidth(), y + fab.getMeasuredHeight());
            } else {
                fab.layout(x, y, x + fab.getMeasuredWidth(), y + fab.getMeasuredHeight());
            }

            TextView label = labelList.get(i);
            int labelStart = rtl ? labelXNearButton + label.getMeasuredWidth() : labelXNearButton - label.getMeasuredWidth();
            int labelEnd = labelXNearButton;
            int labelTop = y + (fab.getMeasuredHeight() - label.getMeasuredHeight()) / 2;
            if (rtl) {
                label.layout(labelEnd, labelTop, labelStart, labelTop + label.getMeasuredHeight());
            } else {
