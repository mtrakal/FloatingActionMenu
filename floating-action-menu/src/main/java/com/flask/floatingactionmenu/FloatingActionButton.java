...
public class FloatingActionButton extends ImageButton {
    ...
    protected void init(Context context, AttributeSet attrs) {
        shadowSize = getDimension(R.dimen.fam_shadow_size);
    ...
    protected void initAttributes(Context context, AttributeSet attributeSet) {
        ...
        if (attr != null) {
            try {
                colorNormal = attr.getColor(R.styleable.FloatingActionButton_fam_colorNormal, getColor(R.color.inbox_fab));
                if (attr.getBoolean(R.styleable.FloatingActionButton_fam_defaultPressedColor, false) || !attr.hasValue(R.styleable.FloatingActionButton_fam_colorPressed))
                    setBackgroundColorInner(colorNormal, true);
                else
                    colorPressed = attr.getColor(R.styleable.FloatingActionButton_fam_colorPressed, getColor(R.color.inbox_fab_pressed));
                colorDisabled = attr.getColor(R.styleable.FloatingActionButton_fam_colorDisabled, colorDisabled);
                normalIcon = attr.getResourceId(R.styleable.FloatingActionButton_fam_normal_icon, 0);
                type = attr.getInt(R.styleable.FloatingActionButton_fam_type, TYPE_NORMAL);
                labelText = attr.getString(R.styleable.FloatingActionButton_fam_labelText);
            } finally {
                attr.recycle();
            }
        }
    ...
    protected void updateBackground() {
        ...
        int circleSize = getDimension(type == TYPE_NORMAL ? R.dimen.fam_size_normal : R.dimen.fam_size_mini);
        int inset = (circleSize - getDimension(type == TYPE_NORMAL ? R.dimen.fam_icon_size : R.dimen.fam_icon_size_mini)) / 2;
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int size = getDimension(type == TYPE_NORMAL ? R.dimen.fam_size_normal : R.dimen.fam_size_mini);
        size += shadowSize * 2;
